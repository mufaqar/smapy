...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full relative items-center group justify-between rounded-md border focus:border-black outline-none right-0 border-slate-300 bg-transparent text-slate-400 focus:text-black py-2 px-3 text-sm placeholder:text-slate-400 focus:outline-none  disabled:cursor-not-allowed disabled:opacity-50",
      className
    )}
    {...props}
  >
    {children}
    <ChevronDown className="h-6 w-6 absolute right-2 opacity-50" />
  </SelectPrimitive.Trigger>
));
